...
function Todos({isAuthenticated, setIsAuthenticated}) {
	...
	useEffect(() => {
		const loadData = async () => {
			let response = null;
			try {
				let url = `http://localhost:8081/api/todo/${pageNumber - 1}/${pageSize}`;

				if(filter === 'Completed'){
					url = `http://localhost:8081/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=true`;
				} else if(filter === 'Not Completed'){
					url = `http://localhost:8081/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=false`;
				}
	...
	const markCompleted = async (id) => {
		try {
      await axios.put(`http://localhost:8081/api/todo/${id}/markcomplete`, {}, {
				headers: {
