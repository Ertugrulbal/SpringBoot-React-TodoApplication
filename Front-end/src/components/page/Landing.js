...
export default function Landing({isAuthenticated, setIsAuthenticated}) {
  ...
  useEffect(() => {
    async function getAndSetNumberAllTodo() {
      try{
        const response = await axios.get('http://localhost:8081/api/todo/count', {
          headers: {
    ...
    async function getAndSetNumberAllTodoNotCompleted() {
      try{
        const response = await axios.get('http://localhost:8081/api/todo/count?isCompleted=false', {
          headers: {
            'Authorization': `Bearer ${sessionStorage.getItem('token')}`,
          }
        });
	...
	return (
		<div className="text-center">
			<h1>Todo List Application for JForce</h1>
      {showErrorMessage()}
			{message}
		</div>
	)
